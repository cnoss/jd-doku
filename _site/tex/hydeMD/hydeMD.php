...
class Document {
  ...
  private function fetchFiles() {
    if( $this->type === self::FILE ) {
      ...
      ### sort files, so that doc is in order (by matthias)
      ksort($this->files);
  ...
  private function prepareAttachments() {
    ...
    foreach( $attachments as $attachment ) {
      ...
      $href = str_replace( '../anhaenge', 'https://coco.dock.moxd.io/anhaenge', $href );
      $href = str_replace( '../download', 'https://coco.dock.moxd.io/download', $href );
  ...
  $latexContent = preg_replace( '/\\\\{\\\\% cite (.*?) \\\\%\\\\}/', "\cite{"."$1"."}", $latexContent);

  /* Links ins Repo */
  $latexContent = preg_replace( '=href{\.\./anhaenge=', "href{https://coco.dock.moxd.io/anhaenge", $latexContent);
  $latexContent = preg_replace( '=href{\.\./download=', "href{https://coco.dock.moxd.io/download", $latexContent);
